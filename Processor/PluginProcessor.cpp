...
void AudioPluginAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    leftChain.prepare(spec);                                                                        // Prepare context with samplesrate usw
                                              ...
                                              juce::MidiBuffer& midiMessages)
{
    ...
    juce::dsp::ProcessContextReplacing<float> leftContext(leftBlock);                         // wrap the block
    juce::dsp::ProcessContextReplacing<float> rightContext(rightBlock);

    leftChain.process(leftContext);                                                      // process Block
    rightChain.process(rightContext);                                                    // process Block
...
ChainSettings getChainSettings(juce::AudioProcessorValueTreeState& apvts)                           // Connect Parameter with Settings
{
...
{
    // LowPass
    auto lowCutCoefficion = makeLowCutFilter(chainSettings, getSampleRate());
    ...
    updateCutFilter(leftLowCut, lowCutCoefficion, chainSettings.lowCutSlope);
    updateCutFilter(rightLowCut, lowCutCoefficion, chainSettings.lowCutSlope);
...
{
    // HighCut
    auto highCutCoefficion = makeHighCutFilter(chainSettings,getSampleRate());
    ...
    updateCutFilter(rightHighCut, highCutCoefficion, chainSettings.highCutSlope);
    updateCutFilter(leftHighCut, highCutCoefficion, chainSettings.highCutSlope);
